class CreatePersonWidgetMigration < ::Scrivito::Migration
  def up
    ...
    p.update(content: p.content << ThreeColumnWidget.new())
    p.content[2].update(column_1: [] << PersonWidget.new(person: Obj.find_by_path("/people/person1"), show_picture: "no"))
    p.content[2].update(column_2: [] << PersonWidget.new(person: Obj.find_by_path("/people/person0"), show_picture: "no"))
  end
end
